...
date: "2018-04-20"
title: "AdWords Scripts: Automatically Create Ads"
tags: ["adwords", "scripts", "adwords scripts", "automatically", "create entities", "create ad groups", "create keywords", "create ads"]
---

# How to create entities from AdWords Scripts?
Today I'm going to introduce a new concept from AdWords Scripts: `Builders`!
For a reason unknown to me, builders are available for ad groups, keywords and ads but not campaigns...
Though I might share with you a few tricks later on how to circumvent that 😉.

__For the purpose of this tutorial, let's say we own bike repair shops in every major cities in the United States and want to automatically generate ads for every single repair shop.__

Let's start off with a new script. I'll personnally call it "new US cities".
...
I went ahead and created a new search campaign manually called `united_states-cities` as AdWords won't allow us to create a campaign through AdWords Scripts yet. You can also use an existing campaign.
...
Once we selected in which campaign we want to create our new ad groups, this is how adding a new ad group is done:
```javascript
var campaign = AdWordsApp.campaigns()
    .withCondition("Name = 'united_states-cities'")
    .withLimit(1)
...
It is also possible to read such a list from a Google Spreadsheet but I'll leave it for another tutorial 😉.
...
Now, that is all fine and dandy, but when you like standardization like me, you'd want to have ad groups that are more computer-readable, so for example, having `new_york_city` instead of `New York City` as an ad group name. We can simply lowercase the name and replace spaces with underscores (`javascript›city.toLowerCase().replace(/ /g, '_')`).
...
Doesn't sound too complicated, except a lot of American city names include spaces (and I'm not even talking about French city names that can be hyphenated (like Aix-en-Provence in France or Saint-Jérôme in Québec) but I have a trick to handle that. By prepending `+` before our keyword and replace every space by `[space]+`, we should have our broad match modifier keyword.

Example: `javascript›Logger.log('+' + 'bike repair Chicago'.replace(/ /g, ' +'));` does get us `+bike +repair +chicago`!
...
Now take a look at the preview results will you?

![keyword builder preview](kw_builder_preview.png)

# 😍

Improvements of this part of the script would be to include more keywords (like "bicycle shop" instead of only "bike shop") as well as adding phrase match keywords & exact match keywords but you should be able to figure it out 😉. Remember that AdWords can infer the match type through the keyword text, for example: `"bike repair shop chicago"` is a phrase match keyword and `[bike repair shop chicago]` is an exact match keyword so only need to prepend and append either double quotes or square brackets arround our keyword text template and that's it!
...
- Each headline have a 30 characters limit
- Description line have a 80 characters limit
...
Pretty amazing that we created these ads automatically. It would be little to no effort to scale them to include any number of cities we wanted! Last step is to unpause your ad groups whenever you're ready to launch 🚀.
